...
After applying six different imputation methods, different results were obtained. The results do not vary significantly, they all follow certain trend, but they are still different. It is important to note that only the data points subjected to imputation (the periods of stockouts) are plotted to avoid cluttering the graph with the extensive actual sales data, which would make the differences in imputed values less observable. Each method is represented by a distinct line color on the graph. There are periods where the differences between the interpolation methods are more significant. For example, at the beginning of 2019, there are noticeable differences among the methods, as we can spot in the graph above. This is because there is least data to observe. As we go further, they became more similar, since quantity of data provides clear trend, and every method will manage to catch some of it. This again emphasizes the importance of having enough data. Some methods may consistently produce higher or lower values compared to others. 
...
Drawing conclusions from my research, the approximation methods showcased remarkable performance, with all reporting an accuracy surpassing 90%. Some even reached an accuracy of approximately 97%. Two methods stood out notably: the Holt-Winters method (also termed as 'triple exponential smoothing') and the SARIMA method which was chosen by auto.arima function from R. Their efficacy was anticipated given the presence of both trend and seasonality in our data. My research further showed that I can rely on selected approximation methods for data characterized by seasonality and trend. Consequently, this led to precise forecasting when combined with forecasting methods such as SARIMA and Holt-Winters. I choose these approximation methods based on this specific dataset and I choose only those that can handle seasonality patterns and trend. That is why every approximation methods have similar result, and there are no situations where some of the approximation methods got completely false results.

For businesses to achieve optimal demand forecasting, a comprehensive understanding of their data is crucial. Recognizing elements such as seasonality and when does it occur, trend magnitude, anomalies such as stockouts, and similar is essential. For instance, while auto.arima displayed significant success, it was not as easy as just applying the code to the data. When deployed with our approximated data without specific frequency, it failed to capture seasonality, choosing a regular ARIMA method. Once I specified its frequency parameters, its performance dramatically improved.

An additional important recommendation for the companies is to keep track of their inventory levels, so they can flag shortages as they occur. This practice holds significant importance because relying on manual detection of shortages is not a good practice. For simplicity of problem demonstration, I assumed that the days with 0 sales are the only ones with the stockouts. But we can have some sales that day and then have a stockout. Of course, the method can be trained to recognize some small sales and mark them as outliers, that is, potential stockouts, but there are better practices than this. The perfect case for the companies would be to try to collect all the demand and then make a forecast based on the demand and not sales
