...
calculate_approximate_demand <- function(data, stockout_index) {
  ...
  # Calculate the start date (two months before the stockout)
  start_date <- stockout_date %m-% months(2)
  
  # Calculate the end date (one day before the stockout)
  end_date <- stockout_date - days(1)
  
  # Filter data for non-stockout days within the last two months, excluding May, June, and July
  relevant_data <- data %>%
